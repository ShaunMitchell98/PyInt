...
static int WriteLocalInstruction(IOSettings* settings, const char* instructionName, char* buffer, int bufferSize, uint8_t constant, char* localName, int localLength, int offset) {
    strcat_s(buffer, bufferSize, instructionName);
    strcat_s(buffer, bufferSize, "\t\t ");
    char text[10] = "\0";
    _itoa_s(constant, text, 10, 10);
...
static int WriteClosureOperation(IOSettings* settings, char* buffer, int bufferSize, Bytecode* bytecode, int offset) {
    offset++;
...
static int WriteBytecodeInstruction(IOSettings* settings, Local* stack, Bytecode *bytecode, char* buffer, int bufferSize, int offset) {
    ...
    switch (instruction) {
        ...
        case SET_LOCAL_OP:
            constant = bytecode->code[offset+1];
            offset= WriteLocalInstruction(settings, "SET_LOCAL_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        case GET_LOCAL_OP:
            constant = bytecode->code[offset+1];
            offset = WriteLocalInstruction(settings, "GET_LOCAL_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        case GET_UPVALUE_OP:
            constant = bytecode->code[offset + 1];
            offset = WriteLocalInstruction(settings, "GET_UPVALUE_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        case SET_UPVALUE_OP:
            constant = bytecode->code[offset + 1];
            offset = WriteLocalInstruction(settings, "SET_UPVALUE_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        case END_OF_ARRAY_OP:
            constant = bytecode->code[offset+1];
            offset = WriteLocalInstruction(settings, "END_OF_ARRAY_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        case GET_INDEX_OP:
            constant = bytecode->code[offset+1];
            offset = WriteLocalInstruction(settings, "GET_INDEX_OP", buffer, bufferSize, constant, (stack + constant)->name.start, (stack + constant)->name.length, offset);
            break;
        default:
...
static int WriteExecutionInstruction(IOSettings* settings, Value* stack, Bytecode* bytecode, char* buffer, int bufferSize, int offset) {
    ...
    case SET_LOCAL_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "SET_LOCAL_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    case GET_LOCAL_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "GET_LOCAL_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    case GET_UPVALUE_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "GET_UPVALUE_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    case SET_UPVALUE_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "SET_UPVALUE_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    case END_OF_ARRAY_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "END_OF_ARRAY_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    case GET_INDEX_OP:
        constant = bytecode->code[offset + 1];
        local = AS_STRING(*(stack + constant));
        offset = WriteLocalInstruction(settings, "GET_INDEX_OP", buffer, bufferSize, constant, local->chars, local->length, offset);
        break;
    default:
        offset = WriteInstructionWithoutConstant("Unknown Opcode", buffer, bufferSize, offset);
        break;
...
static int DisassembleBytecodeInstruction(IOSettings* settings, Local* stack, Bytecode* bytecode, char* buffer, int bufferSize, int offset) {
    strcat_s(buffer, bufferSize, "\t");
    ...
    strcat_s(buffer, bufferSize, "\t  ");
    offset = WriteBytecodeInstruction(settings, stack, bytecode, buffer, bufferSize, offset); 
...
void DisassembleBytecode(Local* stack, Bytecode* bytecode, const char* functionName, IOSettings* bytecodeSettings) {
    ...
    *buffer = '\0';
    ...
    for (int offset = 0; offset < bytecode->count;) {
        offset = DisassembleBytecodeInstruction(bytecodeSettings, stack, bytecode, buffer, bufferSize, offset);
        strcat_s(buffer, bufferSize, "\n");
    }
